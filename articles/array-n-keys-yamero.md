...
この記事で暑かったようなemptyを含む配列は、emptyを含まない配列に比べてパフォーマンス上不利であることが知られています（[例えばV8のブログ記事をご覧ください](https://v8.dev/blog/elements-kinds)）。この記事では理解しやすさの観点から議論しますが、パフォーマンスの観点からもemptyを含む配列を無闇に扱うべきではないことが分かります。
