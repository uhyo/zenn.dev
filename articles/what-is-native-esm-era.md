...
ただし、余談ですが、何も工夫をしない状態ではファイルの変更はページの再読み込み（＝プロジェクト全体の再配信）を引き起こします。これをより最適化するためには**HMR境界**を定義する必要があります（viteを例に説明します。他のツールでは用語が異なるかもしれません）。HMR境界とは、「このモジュール（およびその依存モジュール）が書き換えられたときはこのモジュールだけ再読み込みすればよくて、このモジュールに依存するファイルたちは再読み込みをする必要がない」という宣言です。HMR境界があれば、その内側のモジュールが更新されたときは境界の内側のみ再読み込みし、外側はそのままということが可能です。HMR境界は開発効率の向上のために重要です。
