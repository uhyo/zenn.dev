...
ES Modulesが使われたプロジェクトをバンドラによって1つの非ES Modulesなファイルにまとめることは、実行時パフォーマンスの問題もあります。現在のバンドラによる成果物には、元々のソースファイルにあった`import`をエミュレートするためのランタイムが含まれています。つまり、モジュールグラフの情報がそのランタイムに隠されており、ブラウザは直接モジュールグラフを認識できないということです。
