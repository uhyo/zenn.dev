...
つまり、`await`のようにPromiseから中身を取り出すのが`use`の役目ということです。もちろん、`use`に渡されたPromiseが未解決だった場合はコンポーネントのサスペンドが発生します。

従来のSuspenseと異なる点は、従来はReactは「投げられたPromiseが解決したら再度レンダリングを試みる」ことのみをサポートしており、Promiseの中身を取り出すことは我々に任されていました。それに加えて、`use`ではPromiseの中身を取り出してくれるところまでやってくれるのが新しい点です。
...
つまり、「すでに読み込み中のデータを再度fetchしようとした場合はキャッシュ済みのデータを返す」というキャッシュの機構はいまだに必要だということです。
...
Reactはこれまで、サードパーティライブラリと協調しながら何をReactのコアに導入すべきか慎重に検討してきました。今回のRFCもその流れに乗り、サードパーティライブラリの負担を軽減し