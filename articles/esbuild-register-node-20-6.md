...
以上の実装がNode.js 20.6.0で動かなくなった理由は、**`load`フックから`format: 'commonjs'`が返されたときに、`source`を使う機能が追加された**からです。この機能追加により、CommonJSの読み込みに対しても、`load`フックのようなAPIが使われるようになります。従来のCommonJS向けのフックの実装は、Node.jsの内部実装を直接モンキーパッチするような行儀の悪いものでした。Node.js 20.6.0の新機能により、CommonJSに対してもより安定した、公式のAPIを使えるようになったということです。

言い換えると、従来は`load`フックは「このファイルはCommonJSだよ」と教える役割しか無かったところ、Node.js 20.6.0では「このファイルはCommonJSで、中身はこうだよ」と教える役割になったということです。
