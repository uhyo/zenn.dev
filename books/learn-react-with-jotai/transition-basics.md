...
トランジションは、よく考えると不思議な挙動を示します。ボタンを押してサスペンドしたということは、Reactは間違いなく`userId`が`"user2"`であるUIをレンダリングしたはず（サスペンドにより中断されましたが）です。しかし、それとは別に`userId`が`"user1"`であり、かつ`isPending`が`true`に変わった状況のUIもレンダリングしています。
...
冒頭で、トランジションは「優先度が低い更新」として扱われると説明しました。どうしてそれが、サスペンド中古いUIが表示されるという挙動になるのでしょうか。考えてみましょう。
...
このように、更新の優先度が低い場合、素早く新しいステートを画面に表示するよりも、完全なUIができあがるまで待つことを選択します。これがトランジションと更新の優先度の関係です。

### トランジションのレンダリングの順番

ちょっと細かい裏側の話です。先ほど説明したように、トランジション発生時にはReactは2つの世界のUIを同時にレンダリングしています。「`isPending`が`true`になった古い世界」と、「`userId`が新しい値に変わった新しい世界」です。この2つは、どちらが先にレンダリングされるのでしょうか？
...
ただし、ここで言うレンダリングされるというのは、ReactがUIの計算処理を行うということです。それが画面に反映されるかは別の問題です。

今回の場合、トランジションの更新（`userId`が`user2`になる更新）がサスペンドしなかった場合、すぐに新しい世界の画面が反映されるので、古い世界のUIは画面に反映されることはなく、計算が無駄になります。しかし、トランジションは優先度の低い更新なので、多少無駄があってもよいのです。そのため、Reactはサスペンドするかどうか分からなくても、トランジションした場合に備えた計算を行うのです。
