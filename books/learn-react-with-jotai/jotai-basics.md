...
このように、書き込み可能なatomは、定義次第で引数の受け取り方を返ることができるのです。
...
そのようなユーティリティ関数は我々で実装することもできますし、一部はjotaiやその周辺ライブラリから提供されちえます。例えば、次のページでは「リセット可能」なatomを定義できる`atomWithReset`が紹介されています。
