...
典型的には、Error Boundaryは**リセット**できる必要があります。リセットというのは、エラーをキャッチしてフォールバックUIを表示している状態から、再度子コンポーネントが表示される状態へ以降することです。
...
一工夫とは、**ストアを直接操作する**ことです。jotaiでは、atomのステートが実際に保管されている場所はストアというオブジェクトです。スト
アを直接操作すれば、`useAtom`のようなフックを経由しなくてもステートを読み書きできます。
